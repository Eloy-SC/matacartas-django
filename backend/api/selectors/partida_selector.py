...
def get_partidas_publicas_count():
    return Partida.objects.filter(privada=False, fecha_creacion__isnull=False, fecha_inicio__isnull=True).count()

def get_partidas_publicas_paginated(offset, limit):
    return Partida.objects.filter(privada=False, fecha_creacion__isnull=False, fecha_inicio__isnull=True).order_by("id")[offset:offset+limit]

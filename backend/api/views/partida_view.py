...
def crear_partida(request):
    ...
    try:
        partida = partida_service.crear_partida(
            ...
            cartas_especiales=serializer.validated_data["cartas_especiales"],
...
@api_view(["PUT"])
@permission_classes([IsAuthenticated])
def iniciar_partida_manual(request, partida_id):
    try:
        partida_service.iniciar_partida(request.user, partida_id, manual=True)
    except PermissionError as e:
        return Response({"detail": str(e)}, status=403)
    except ValueError as e:
        return Response({"detail": str(e)}, status=404)
